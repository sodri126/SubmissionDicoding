...
class MovieHolder(view: View, private val onClickItem: IClickItem): RecyclerView.ViewHolder(view) {
    ...
    fun bind(movie: Any, deleteItem: () -> Unit) {
        ...
        when(movie) {
            is DiscoverMovie -> {
                imgMovie.glideLoadImage("${BuildConfig.PATH_IMG}/w780${movie.posterPath}", 780, 500, progressbarImg)
                txtTitleMovie.text = if (movie.title.length >= 24) "${movie.title.substring(0, 24)}..." else movie.title
                txtReleaseDate.text = sdf.format(movie.releaseDate)
                txtContent.text = if (movie.overview.length > 130) "${movie.overview.substring(0, movie.overview.substring(0, 130).lastIndexOf(' '))}..." else movie.overview
            ...
            is DiscoverTv -> {
                imgMovie.glideLoadImage("${BuildConfig.PATH_IMG}/w780${movie.posterPath}", 780, 500, progressbarImg)
                txtTitleMovie.text = if (movie.name.length >= 24) "${movie.name.substring(0, 24)}..." else movie.name
            ...
            is MovieEntity -> {
                imgMovie.glideLoadImage("${BuildConfig.PATH_IMG}/w780${movie.movieImagePath}", 780, 500, progressbarImg)
                txtTitleMovie.text = if (movie.movieTitle.length >= 24) "${movie.movieTitle.substring(0, 24)}..." else movie.movieTitle
                txtReleaseDate.text = movie.movieDate
                txtContent.text = if (movie.movieOverview.length > 130) "${movie.movieOverview.substring(0, movie.movieOverview.substring(0, 130).lastIndexOf(' '))}..." else movie.movieOverview
            ...
            is TvShowEntity -> {
                imgMovie.glideLoadImage("${BuildConfig.PATH_IMG}/w780${movie.tvShowImagePath}", 780, 500, progressbarImg)
                txtTitleMovie.text = if (movie.tvShowTitle.length >= 24) "${movie.tvShowTitle.substring(0, 24)}..." else movie.tvShowTitle
                txtReleaseDate.text = movie.tvShowDate
                txtContent.text = if (movie.tvShowOverview.length > 130) "${movie.tvShowOverview.substring(0, movie.tvShowOverview.substring(0, 130).lastIndexOf(' '))}..." else movie.tvShowOverview
